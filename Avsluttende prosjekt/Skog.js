...
let random_index;
  ...
  function get_random_image(){
    ...
    // Display the image
    document.getElementById('image_shower').src = `../Bilder/${selected_image}`
    svarEl.value = ""
...
sjekkBtn.addEventListener('keypress', function(e){
    if(event.key === 'Enter'){
        sjekkSvar()
        console.log("hbfeb")
    }
...
function sjekkSvar(){
    ...
    if(svar === riktig){
        ...
        if (streak === 5){
            document.getElementById('image_shower').src = `../Bilder/fempaarad.png`;
            resultatEl.innerHTML = "Riktig!! Du har 5 på rad"
            
        }
        else if (streak === 15){
            document.getElementById('image_shower').src = `../Bilder/gullstjerne.jpeg`;
